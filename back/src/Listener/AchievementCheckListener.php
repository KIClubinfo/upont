...
class AchievementCheckListener
{
    ...
    {
        $this->user = $user;
        $repoAU = $this->manager->getRepository(AchievementUser::class);
    ...
    public function check(AchievementCheckEvent $event)
    {
        ...
        $repoA = $this->manager->getRepository(Achievement::class);
    ...
    private function totalPontHubSize()
    {
        ...
        FROM App:PonthubFileUser pfu,
        App:PonthubFile file
        WHERE pfu.user = :user
        AND pfu.file = file')
    ...
    {
        $repo = $this->manager->getRepository(ClubUser::class);
