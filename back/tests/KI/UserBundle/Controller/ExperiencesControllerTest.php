<?php

namespace Tests\KI\UserBundle\Controller;

use Tests\KI\CoreBundle\WebTestCase;

class ExperiencesControllerTest extends WebTestCase
{
    // On crée une ressource sur laquelle seront effectués les tests. Ne pas oublier de supprimer à la fin avec le test DELETE.
    public function testPost()
    {
        $this->client->request('POST', '/experiences', ['name' => 'Form finding', 'startDate' => 51561, 'endDate' => 41564556, 'description' => 'Super stage', 'country' => 'Espagne', 'city' => 'Madrid', 'category' => 'Stage Chiant', 'company' => 'Universidad Polytecnica de Madrid', 'latitude' => 58.11, 'longitude' => 42.2]);
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, 201);
        // On vérifie que le lieu du nouvel objet a été indiqué
        $this->assertTrue($response->headers->has('Location'), $response->headers);
    }

    public function testGet()
    {
        $this->client->request('GET', '/experiences');
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, 200);

        $this->client->request('GET', '/experiences/form-finding');
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, 200);

        $this->client->request('GET', '/experiences/sjoajsiohaysahais-asbsksaba7');
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, 404);

        $this->client->request('GET', '/users/taquet-c/experiences');
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, 200);

        $this->client->request('GET', '/users/dfhsth/experiences');
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, 404);

        $this->client->request('POST', '/users/dfhsth/experiences');
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, 405);
    }

    public function testPatch()
    {
        $this->client->request('PATCH', '/experiences/form-finding', ['endDate' => 454523]);
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, 204);

        $this->client->request('PATCH', '/experiences/form-finding', ['name' => '']);
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, 400);

        $this->client->request('PATCH', '/experiences/sjoajsiosbsksaba7', ['name' => 'miam', 'mail' => '123@mail.fr']);
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, 404);

        $this->client->request('PATCH', '/experiences/form-finding', ['name' => 'miam', 'mail' => '123@mail.fr']);
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, 400);
    }

    public function testDelete()
    {
        $this->client->request('DELETE', '/experiences/form-finding');
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, 204);

        $this->client->request('DELETE', '/experiences/form-finding');
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, 404);
    }
}
